...
fn main() {
    ...
    let python_installed = Command::new("sh").args(&["-c", "command -v python3"]).status().unwrap().success();
    let pip_installed = Command::new("sh").args(&["-c", "command -v pip3"]).status().unwrap().success();
    let wget_installed = Command::new("sh").args(&["-c", "command -v wget"]).status().unwrap().success();
    
    if python_installed && pip_installed && wget_installed {
        Command::new("pip3").args(&["install", "aiohttp", "toml"]).status().unwrap();
        ...
        Command::new("python3").args(&["flatpak-cargo-generator.py", "Cargo.lock", "-o", "src/resources/cargo-sources.json"]).status().unwrap();
        Command::new("rm").arg("flatpak-cargo-generator.py").status().unwrap();
    }
